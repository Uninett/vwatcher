...
INITIAL_UPTIME_SECONDS = 3 * 24 * 60 * 60  # 3 days
...
class TestTwoDaysInOneDevicesLife:
    async def test_should_report_upgrade_and_then_crash(self, watched_device, log_tree, clock):
    ...
    async def test_should_reproduce_rotated_days_report_on_demand(self, watched_device, log_tree):
    ...
    async def test_when_device_just_stays_up_then_it_should_keep_quiet(self, watched_device, log_tree):
    ...
    async def test_when_device_restarted_unwatched_then_it_should_be_noticed(self, watched_device, log_tree):
        await watched_device.poll(times=3)
