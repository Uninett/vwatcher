...
def get_events(log_tree, event=None):
    return [entry for entry in log_tree.get_entries() if event is None or entry.event == event]


class TestFirstPoll:
    async def test_should_log_restart_because_the_device_is_unknown(self, task, log_tree):
        await task.run()
        assert [entry.event for entry in get_events(log_tree)] == [RELOADED, UPTIME, SOFTWARE, RESTART_REASON]
    ...
    async def test_should_log_when_the_device_booted(self, task, log_tree, session, clock):
        await task.run()
        booted = parse_timestamp(get_events(log_tree, RELOADED)[0].value)
...
class TestSubsequentPolls:
    async def test_when_uptime_grows_as_expected_then_nothing_should_be_logged(self, task, log_tree, session, clock):
        await task.run()
        logged = len(get_events(log_tree))

        clock.advance(300)
        session.uptime += 300 * TICKS_PER_SECOND
        await task.run()

        assert len(get_events(log_tree)) == logged

    async def test_when_uptime_dropped_then_restart_should_be_logged(self, task, log_tree, session, clock):
        await task.run()
        clock.advance(300)
        session.uptime = 60 * TICKS_PER_SECOND
        await task.run()

        assert [entry.event for entry in get_events(log_tree)][-4:] == [RELOADED, UPTIME, SOFTWARE, RESTART_REASON]
    ...
    async def test_when_uptime_lags_beyond_the_allowed_offset_then_restart_should_be_logged(
        self, task, log_tree, session, clock
    ):
        await task.run()
    ...
    async def test_when_the_device_was_just_booting_then_the_next_poll_should_confirm_the_restart(
        ...
        assert len(get_events(log_tree, RELOADED)) == 2


class TestCounterWraparound:
    async def test_should_not_be_reported_as_restart(self, task, log_tree, session, clock):
        await task.run()
        session.uptime = COUNTER32_MAX - task.allowed_offset
        clock.advance(60)
        session.uptime += 60 * TICKS_PER_SECOND
        await task.run()
        reloads = len(get_events(log_tree, RELOADED))
        ...
        assert len(get_events(log_tree, RELOADED)) == reloads
...
class TestRestartReason:
    async def test_should_be_polled_for_cisco_devices(self, task, log_tree, session):
        await task.run()
        assert get_events(log_tree, RESTART_REASON)[0].value == session.why_reload
    ...
    async def test_should_not_be_polled_for_other_vendors(self, device, state, log_tree, clock):
        ...
        assert get_events(log_tree, RESTART_REASON) == []
...
class TestSnmpFailure:
    ...
    async def test_when_the_uptime_poll_recovers_then_the_device_is_not_reported_restarted_twice(
        ...
        assert len(get_events(log_tree, RELOADED)) == 1
