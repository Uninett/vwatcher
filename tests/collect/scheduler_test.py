...
# The defaults from the `polldevs_config` fixture, so that a rewrite only changes
...
class TestLoadPolldevs:
    def test_should_report_every_device_as_new_on_the_first_read(self, poller, polldevs_config):
        new, deleted, changed, defaults = poller.load_polldevs(polldevs_config)

        assert new == {"example-gw", "example-gw2"}
        assert not deleted and not changed
        assert defaults["interval"] == "5"

    def test_when_the_file_is_untouched_then_it_should_report_no_changes(self, poller, polldevs_config):
        poller.load_polldevs(polldevs_config)

        assert poller.load_polldevs(polldevs_config) == (set(), set(), set(), {})

    def test_should_report_removed_device_as_deleted(self, poller, polldevs_config):
        poller.load_polldevs(polldevs_config)
        rewrite(polldevs_config, "name: example-gw\naddress: 10.0.42.1\n")

        new, deleted, changed, _ = poller.load_polldevs(polldevs_config)

        assert deleted == {"example-gw2"}
        assert not new and not changed

    def test_should_forget_the_state_of_deleted_device(self, poller, polldevs_config, state):
        poller.load_polldevs(polldevs_config)
        state["example-gw2"]
        rewrite(polldevs_config, "name: example-gw\naddress: 10.0.42.1\n")
    ...
    def test_should_report_edited_device_as_changed(self, poller, polldevs_config):
        poller.load_polldevs(polldevs_config)
        rewrite(polldevs_config, "name: example-gw\naddress: 10.0.42.1\ninterval: 10\n")
    ...
    def test_when_the_file_is_missing_then_it_should_report_nothing(self, poller, polldevs_config):
        polldevs_config.unlink()

        assert poller.load_polldevs(polldevs_config) == (set(), set(), set(), {})

    def test_when_the_file_turns_invalid_then_it_should_keep_the_devices_it_had(self, poller, polldevs_config):
        poller.load_polldevs(polldevs_config)
        rewrite(polldevs_config, "this is not a setting\n")
...
class TestScheduling:
    def test_should_schedule_job_per_device(self, poller, scheduler):
        poller.reload_polldevs()

        assert set(scheduler.jobs) == {"example-gw", "example-gw2"}

    def test_should_poll_device_at_its_own_interval(self, poller, scheduler):
        poller.reload_polldevs()
    ...
    def test_should_deschedule_deleted_device(self, poller, scheduler, polldevs_config):
        poller.reload_polldevs()
        rewrite(polldevs_config, "name: example-gw\naddress: 10.0.42.1\n")
    ...
    def test_should_reschedule_changed_device(self, poller, scheduler, polldevs_config):
        poller.reload_polldevs()
        rewrite(polldevs_config, "name: example-gw\naddress: 10.0.42.1\ninterval: 10\n")
    ...
    def test_should_drop_the_session_of_descheduled_device(self, poller):
        poller.reload_polldevs()
    ...
    def test_when_device_was_never_scheduled_then_descheduling_should_be_tolerated(self, poller):
        poller.deschedule_devices(["never-seen"])


class TestPollDevice:
    async def test_should_poll_configured_device(self, poller, polldevs_config, log_tree):
        poller.load_polldevs(polldevs_config)
    ...
    async def test_should_reuse_one_session_per_device(self, poller, polldevs_config):
        poller.load_polldevs(polldevs_config)
    ...
    async def test_when_device_is_gone_then_it_should_do_nothing(self, poller, log_tree):
...
class TestRun:
    async def test_should_start_the_scheduler_and_schedule_pollfile_reload(self, poller, scheduler, monkeypatch):
...
class TestScheduler:
    async def test_should_give_poll_job_the_configured_defaults(self, config, log_tree):
        config.scheduler.misfire_grace_time = 42
        poller = Poller(config=config, tree=log_tree, session_factory=lambda device: FakeSNMPSession())
