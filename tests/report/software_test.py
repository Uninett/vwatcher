...
class TestPrettyDesc:
    ...
    def test_should_condense_a_known_description(self, descr, expected):
        assert pretty_desc(descr) == expected

    def test_should_pass_an_unknown_description_through(self):
        assert pretty_desc("Some Vendor Router") == "Some Vendor Router"

    def test_should_remove_newlines_from_a_wrapped_description(self):
...
class TestSplitPretty:
    def test_should_split_a_condensed_description(self):
        assert split_pretty("Cisco-IOS 15.2(4)S7 C7200") == ("Cisco-IOS", "15.2(4)S7")

    def test_when_a_description_cannot_be_split_then_it_should_return_empty_fields(self):
...
class TestDissectCiscoVersion:
    ...
    def test_should_split_a_version(self, version, expected):
        assert dissect_cisco_version(version) == expected

    def test_when_a_version_is_unrecognized_then_it_should_return_none(self):
...
class TestDissectJunosVersion:
    ...
    def test_should_rank_a_service_release_above_a_plain_revision(self):
...
class TestLeadingNumber:
    ...
    def test_should_format_like_old_zino(self, value, expected):
...
class TestVersionSortKey:
    ...
    def test_should_group_by_operating_system(self):
        devices = [("gw2", "JunOS 20.4R3.8 mx480"), ("gw1", "Cisco-IOS 15.2(4)S7 C7200")]
