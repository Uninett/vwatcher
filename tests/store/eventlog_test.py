...
class TestParseTimestamp:
    def test_should_parse_a_log_timestamp(self):
        assert parse_timestamp(TIMESTAMP) == datetime(2025, 9, 4, 12, 0, 0)

    def test_should_ignore_a_time_zone_in_the_middle(self):
    ...
    def test_should_round_trip_a_formatted_timestamp(self):
...
class TestEventLogWrite:
    ...
    def test_should_write_a_bare_message_without_a_colon(self, tmp_path, clock):
        log = EventLog(tmp_path / "ver-watch.log", clock=clock)
    ...
    def test_should_append_to_an_existing_log(self, tmp_path, clock):
        log = EventLog(tmp_path / "ver-watch.log", clock=clock)
        log.write("one")
        log.write("two")
...
class TestEventLogRead:
    ...
    def test_should_not_mistake_a_fenced_value_for_events(self):
        lines = [
