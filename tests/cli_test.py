...
from vwatcher.store import SOFTWARE
...
class TestVwDaily:
    def test_should_rotate_the_logs_and_mail_the_report(self, vwatcher_conf, busy_day, mailed):
        assert cli.vw_daily(["--config-file", str(vwatcher_conf), "--date", "2025-09-04"]) == 0
...
class TestReportCommands:
    def test_vw_day_report_should_print_a_rotated_days_report(self, vwatcher_conf, busy_day, mailed, capsys):
        cli.vw_daily(["--config-file", str(vwatcher_conf), "--date", "2025-09-04"])
        capsys.readouterr()

        assert cli.vw_day_report(["--config-file", str(vwatcher_conf), "2025-09", "04"]) == 0
        assert "Upgraded routers:" in capsys.readouterr().out

    def test_vw_month_upgr_should_print_the_months_upgrades(self, vwatcher_conf, busy_day, mailed, capsys):
        cli.vw_daily(["--config-file", str(vwatcher_conf), "--date", "2025-09-04"])
        capsys.readouterr()

        assert cli.vw_month_upgr(["--config-file", str(vwatcher_conf), "2025-09"]) == 0
        assert "Sep 04" in capsys.readouterr().out

    def test_vw_month_rst_should_print_nothing_for_a_month_without_restarts(
        self, vwatcher_conf, busy_day, mailed, capsys
    ):
        cli.vw_daily(["--config-file", str(vwatcher_conf), "--date", "2025-09-04"])
        capsys.readouterr()

        assert cli.vw_month_rst(["--config-file", str(vwatcher_conf), "2025-09"]) == 0
        assert capsys.readouterr().out == ""

    @pytest.mark.parametrize("command, with_uptimes", [(cli.cur_vers, False), (cli.uptimes, True)])
    def test_should_print_every_devices_version(self, command, with_uptimes, vwatcher_conf, busy_day, capsys):
        assert command(["--config-file", str(vwatcher_conf)]) == 0

        out = capsys.readouterr().out
        assert "example-gw" in out
        assert ("1:00:00.00" in out) is with_uptimes

    def test_uptimes_should_accept_the_uptime_ordering_flag(self, vwatcher_conf, busy_day, capsys):
        assert cli.uptimes(["--config-file", str(vwatcher_conf), "-u"]) == 0
        assert "example-gw" in capsys.readouterr().out

    def test_when_a_month_was_never_logged_then_it_should_explain_why(self, vwatcher_conf, busy_day, caplog):
        with pytest.raises(SystemExit):
            cli.vw_month_upgr(["--config-file", str(vwatcher_conf), "1999-12"])

        assert "Could not read the logs" in caplog.text


class TestLoadConfig:
    def test_when_there_is_no_config_file_then_it_should_fall_back_to_the_defaults(self, tmp_path, monkeypatch, capsys):
        monkeypatch.chdir(tmp_path)

        assert cli.cur_vers([]) == 0
        assert capsys.readouterr().out == ""

    def test_when_a_named_config_file_is_missing_then_it_should_exit(self, tmp_path, caplog):
        with pytest.raises(SystemExit):
...
class TestVwatcherDaemon:
    ...
    def backends(self, monkeypatch):
        """Ensure a real SNMP back-end is not loaded"""
        loaded = []
    ...
    def test_should_run_a_poller_on_the_configured_pollfile(self, vwatcher_conf, polldevs_conf, pollers):
        assert cli.vwatcher(["--config-file", str(vwatcher_conf)]) == 0
        assert pollers[0].polling.file == str(polldevs_conf)

    def test_should_let_an_argument_override_the_configured_pollfile(self, vwatcher_conf, pollers):
        cli.vwatcher(["--config-file", str(vwatcher_conf), "--polldevs", "/etc/other-polldevs.cf"])

        assert pollers[0].polling.file == "/etc/other-polldevs.cf"

    def test_should_load_the_configured_snmp_backend(self, vwatcher_conf, pollers, backends):
        cli.vwatcher(["--config-file", str(vwatcher_conf)])

        assert backends == ["netsnmp"]

    def test_when_interrupted_then_it_should_exit_quietly(self, vwatcher_conf, monkeypatch):
