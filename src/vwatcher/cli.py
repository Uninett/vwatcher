...
def vw_daily(argv: Optional[Sequence[str]] = None) -> int:
    ...
    parser.add_argument(
        "--date", type=date.fromisoformat, help="The day to rotate into, as YYYY-MM-DD (default: today)"
    )
...
def vw_day_report(argv: Optional[Sequence[str]] = None) -> int:
    ...
    parser.add_argument("year_month", help="Month of the day to report on, as YYYY-MM")
    parser.add_argument("day", help="Day of month to report on, as DD")
    args = _parse_args(parser, argv)
    tree = LogTree(load_config(args).log_directory)
    return _print_report(lambda: reports.day_report(tree, args.year_month, args.day))
...
def _month_report(description: str, build_report, argv: Optional[Sequence[str]]) -> int:
    ...
    parser.add_argument("year_month", help="Month to report on, as YYYY-MM")
    args = _parse_args(parser, argv)
    tree = LogTree(load_config(args).log_directory)
    return _print_report(lambda: build_report(tree, args.year_month))
...
def cur_vers(argv: Optional[Sequence[str]] = None) -> int:
    ...
    tree = LogTree(load_config(args).log_directory)
    return _print_report(lambda: reports.current_versions(tree))
...
def uptimes(argv: Optional[Sequence[str]] = None) -> int:
    ...
    args = _parse_args(parser, argv)
    tree = LogTree(load_config(args).log_directory)
    return _print_report(lambda: reports.uptime_report(tree, by_uptime=args.by_uptime))
...
def load_config(args: argparse.Namespace) -> Configuration:
    ...
    try:
        ...
        if args.config_file:
            ...
            sys.exit(1)
        _log.debug("No configuration file at %s, using the defaults", config_file)
...
def _print_report(build_report) -> int:
    """Build and print a report, exiting the process if the logs cannot be read"""
    try:
