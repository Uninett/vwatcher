...
class DeviceHistory:
    ...
    def get_upgrade_steps(self) -> Iterator[tuple[str, str]]:
...
class Replay:
    ...
    def get_device(self, name: str) -> DeviceHistory:
        """Return a device's history, starting an empty one the first time it is seen"""
        if name not in self.devices:
            self.devices[name] = DeviceHistory(name=name)
        return self.devices[name]

    def sorted_devices(self) -> list[DeviceHistory]:
        """Return every device seen so far, in name order, as the reports list them"""
        return [self.devices[name] for name in sorted(self.devices)]

    def observe_entries(self, entries: Iterable[LogEntry]) -> "Replay":
        """
        Read through the log entries, updating what we know about each device.

        Hands back the Replay itself, so this can follow straight on from creating one.
        """
        for entry in entries:
            self.observe_entry(entry)
        return self

    def observe_entry(self, entry: LogEntry) -> None:
        """Update one device's history from a single log entry"""
        if entry.event == eventlog.RELOADED:
            self._reloaded(entry)
        elif entry.event == eventlog.UPTIME:
            # The value is unused in this case
            self.get_device(entry.device)
    ...
    def observe_software(self, device_name: str, descr: str) -> None:
        """Record a version observation, counting it as an upgrade if it differs from the last"""
        device = self.get_device(device_name)
        if device.current and device.current != descr:
            device.upgrades.append(descr)
        device.current = descr

    def observe_baseline(self, baseline: Mapping[str, str]) -> None:
        """
        ...
        for name in sorted(baseline):
            self.observe_software(name, baseline[name])
    ...
    def flush_upgrades(self, label: str = "") -> list[str]:
        ...
        for device in self.sorted_devices():
            lines += [
                ...
                for previous, version in device.get_upgrade_steps()
            ]
...
def day_report(tree: LogTree, year_month: str, day: str) -> str:
    ...
    day_dir = tree.get_day_dir(year_month, day)
    history = Replay(tree.get_descrs(day_dir)).observe_entries(tree.get_entries(day_dir))
    return format_day_report(history)


def format_day_report(history: Replay) -> str:
    """
    ...
    """
    lines: list[str] = []
    devices = history.sorted_devices()

    if upgraded := [device for device in devices if device.upgraded]:
        lines += ["Upgraded routers:", ""]
        for device in upgraded:
            lines += [
                f"{device.name:<15} from {pretty_desc(previous):>15} to {pretty_desc(version):>15} ({device.restarts})"
                for previous, version in device.get_upgrade_steps()
            ]
        lines.append("")
    ...
    if restarted := [device for device in devices if device.restarted and not _restart_is_boring(device)]:
        ...
        for device in restarted:
            lines.append(f"{device.name:<15} ({device.restarts} total) current version {pretty_desc(device.current)}")
...
def month_upgrade_report(tree: LogTree, year_month: str) -> str:
    """Report every upgrade of one month, one line per version step, in date order"""
    day_dirs = tree.get_day_dirs(year_month)
    if not day_dirs:
        return ""
    lines: list[str] = []
    history = Replay(tree.get_descrs(day_dirs[0]))
    for day_dir in day_dirs:
        history.observe_baseline(tree.get_descrs(day_dir))
        history.observe_entries(tree.get_entries(day_dir))
        lines += history.flush_upgrades(label=_day_label(year_month, day_dir.name))
    return "".join(line + "\n" for line in lines)


def month_restart_report(tree: LogTree, year_month: str) -> str:
    """Report every restart of one month for which a device reported a reason"""
    history = Replay()
    for day_dir in tree.get_day_dirs(year_month):
        history.observe_entries(tree.get_entries(day_dir))
    lines = []
    for event in history.restart_events:
...
def _todays_devices(tree: LogTree) -> list[DeviceHistory]:
    ...
    today = Replay(tree.get_descrs())
    today.observe_entries(tree.get_entries())
    return list(today.devices.values())
...
def _day_label(year_month: str, day: str) -> str:
    """Turn `("2026-09", "04")` into `Sep 04`, the label the monthly report uses"""
    try:
        when = datetime.strptime(f"{year_month}-{day}", "%Y-%m-%d")
    except ValueError:
        return day
...
def rotate_and_report(tree: LogTree, when=None) -> str:
    ...
    report = format_day_report(Replay(tree.get_descrs(day_dir)).observe_entries(tree.get_entries(day_dir)))
